...
const Navbar: React.FC<NavbarProps> = ({ onNavigate, activeSection }) => {
  ...
  // Grouped navigation items with tier requirements
  const calculatorLinks = [
    { id: 'calculator', label: 'Event Footprint', icon: Calculator, requiredTier: 'explorer' as const },
    { id: 'costsavings', label: 'Cost & Savings', icon: DollarSign, requiredTier: 'planner' as const },
  ];

  const resultsLinks = [
    { id: 'dashboard', label: 'Impact Dashboard', icon: LayoutDashboard, requiredTier: 'impact' as const },
    { id: 'alternatives', label: 'Recommendations', icon: Sparkles, requiredTier: 'planner' as const },
  ];
  ...
  const canAccessLink = (requiredTier?: 'explorer' | 'planner' | 'impact' | 'enterprise') => {
    if (!requiredTier) return true;
    const tierHierarchy = { explorer: 0, planner: 1, impact: 2, enterprise: 3 };
    const currentLevel = tierHierarchy[subscriptionTier] || 0;
    const requiredLevel = tierHierarchy[requiredTier] || 0;
    return currentLevel >= requiredLevel;
  };

  // Helper to get tier badge color
  const getTierBadgeColor = (tier: 'explorer' | 'planner' | 'impact' | 'enterprise') => {
    const colors = {
  ...
  const getTierLabel = (tier: 'explorer' | 'planner' | 'impact' | 'enterprise') => {
    const labels = {
      explorer: 'Free',
      planner: 'Planner',
      impact: 'Impact',
            ...
            <div className="hidden lg:flex items-center gap-0.5 xl:gap-1">
              ...
              {isAuthenticated && (
                <div className="relative group">
                  <button
                    onClick={() => {
                      const hasAccess = canAccessLink('planner');
                      if (hasAccess) {
                        onNavigate('myevents');
                      } else {
                        navigate('/pricing');
                  ...
                  >
                    ...
                    My Events
                    {!canAccessLink('planner') && (
                      <span className="ml-1 text-xs px-1.5 py-0.5 rounded-full bg-emerald-100 text-emerald-700">
                        Planner
                      </span>
                    )}
                  </button>
